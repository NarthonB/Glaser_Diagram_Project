...
app.layout = html.Div([
    ...
    html.Div(id='layer-data-table', children=[]),
])
...
@app.callback(
    Output('layer-data-table', 'children'),
...
def process_layer_data(*args):
    ...
    ID = []
    thck = []
    for i in range(min(len(args), MAX_LAYERS)//2):
        ID.append(args[i])
    ...
    Layer_Props = pd.DataFrame(
        ...
        index=['LAYER {}'.format(i+1) for i in range(0,len(args)//2)])
    
    # data = Layer_Props.to_dict('records')
    return html.Div(
        children=[
            dcc.Datatable(
                id='table',
                columns=[{"name": i, "id": i} for i in Layer_Props.columns],
                data=Layer_Props.to_dict('records'),
...
# dcc.DataTable(
    ...
    #     )
    # ]
    # return Layer_Props.to_dict('records')
