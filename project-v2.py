...
to reduce the vapour pressure at this extremity, reducing the likelyhood of condensation """
...
app.layout = html.Div([
    # Importing Font
    html.Link(rel='stylesheet', href='https://fonts.googleapis.com/css2?family=Quicksand:wght@300;400;600&display=swap'),
    # Title
    html.H1("GLASER DIAGRAM",
        style={'text-align': 'center', 'display': 'block', 
               'fontFamily': 'Quicksand', 'fontWeight': '600', 'textSize': '50px'}),
    html.Label("By Nathan Barr",
               style={'text-align': 'center', 'display': 'block', 'fontFamily': 'Quicksand', 'textSize': '14px'}),
    html.Br(),               
    # Intro Text
    html.Div([
        html.Label("""This program estimates the risk of condensation in a wall designed by the user.
        ...
        html.Br(),                    
        html.Label("""After having chosen each layers' material and thickness, the internal conditions are set by the user,
                    while the external conditions are defined by the choice of location and a specified date."""),
        html.Br(),
        html.I("""Note: not all stations have the required station data, so when data is missing, default values are used.""")
    ],style={'textAlign': 'center', 'width': '60%', 'display': 'block', 'margin': '0 auto', 'fontFamily': 'Quicksand', 'textSize': '10px'}), 
    html.Br(),
    html.H2("USER INPUT", style={'text-align': 'center', 'display': 'block', 'fontFamily': 'Quicksand', 'fontWeight': '600'}),
    html.Br(),
    html.Div([
        html.Div([
            html.Div([
                ...
                dash_table.DataTable(
                    ...
                    style_cell={'font-family': 'Quicksand'},
            ...
            html.Div([
                ...
                html.Label('Air Speed [m/s]', style={'text-align': 'center', 'display': 'block', 'textSize': '12px'}),
                dcc.Input(id='internal_air_speed', value=0.2, step=0.1, type='number', style={'display': 'block', 'margin': 'auto', 'margin-top': '5px', 'text-align': 'center', 'textSize': '12px'}),
                html.Label('Temperature [°C]', style={'text-align': 'center', 'display': 'block', 'textSize': '12px'}),
                dcc.Input(id='internal_temperature', value=20.0, type='number', style={'display': 'block', 'margin': 'auto', 'margin-top': '5px', 'text-align': 'center', 'textSize': '12px'}),
                html.Label('Humidity [-]', style={'text-align': 'center', 'display': 'block', 'textSize': '12px'}),
                dcc.Input(id='internal_humidity', value=0.5, step=0.01, type='number', style={'display': 'block', 'margin': 'auto', 'margin-top': '5px', 'text-align': 'center', 'textSize': '12px'}),
            ],style={'margin': '10px', 'text-align': 'center'}),

            ## Location Input
            html.Div([
                html.H3("Location", style={'text-align': 'center', 'display': 'block'}),
                html.Label("Country Code", style={'text-align': 'center', 'margin': 'auto', 'display': 'block'}),
                dcc.Input(id='country-code', value='FR', type='string', style={'display': 'block', 'margin-top': '5px', 'text-align': 'center'}),
                html.Br(),
                html.Label("City/Station", style={'text-align': 'center', 'margin': 'auto', 'display': 'block'}),
                dcc.Dropdown(id='station-dropdown', value='Strasbourg', options=[], style={'display': 'block', 'margin-top': '5px', 'fontSize': '14px'}),
            ...
            html.Div([
                ...
                dcc.DatePickerSingle(
                    ...
                    style={'text-align': 'center', 'margin': '10px', 'display': 'block'}),
                ],style={'margin': '10px', 'fontSize': '16px'}),
            
            ],style={'display': 'flex', 'flex-direction': 'columns', 'align-items': 'center'}), 
        ], style={'display': 'flex', 'flex-direction': 'column', 'align-items': 'center', 'fontFamily': 'Quicksand'}),
    
    html.Div(id='editable-table-output'),
    dcc.Store(id='columns-state', data=Layer_Property_col_names),
    
    html.Br(),
    html.H2("OUTPUT GRAPH", style={'text-align': 'center', 'display': 'block', 'fontFamily': 'Quicksand', 'fontWeight': '600'}),  # center the H2 text
    html.Br(),
    html.Div([
        html.Button('GENERATE', id='calculate-button', n_clicks=1, 
                    style={'height': '50px', 'border': '2px solid black','font-size': '28px',
                        'display': 'flex', 'align-items': 'center', 'justify-content': 'center', 'fontFamily': 'Quicksand'}),
    ], style={'display': 'flex', 'flex-direction': 'column', 'align-items': 'center', 'fontFamily': 'Quicksand'}),

    html.Div([
        dcc.Graph(
            id='Final-Graph',
            style={'width': '120vh', 'height': '80vh'}
            ), 
    ], style={'display': 'flex', 'justify-content': 'center', 'align-items': 'center'}),

    html.Label("If condensation occurs, choose a Vapour Barrier: ", title=Vapour_Barrier_Info,
                style={'display': 'flex', 'justify-content': 'center', 'align-items': 'center', 'fontFamily': 'Quicksand'}),
    html.Div([
    ...
    html.Br(),
    html.Div([
        html.Label("All data and equations were taken from the Applied Thermodynamics course taken at INSA Strasbourg:"),
        html.Br(),
        html.I("""Walther, E. (2024). Thermodynamique Appliquée; Air Humide et Transfert de Masse [Class Handout]. 
                Retrieved from https://moodle.insa-strasbourg.fr/course/view.php?id=1251"""),                
        html.Br(),
        html.Label(["Code can be found on ", html.A("my GitHub page", href="https://github.com/NateB555/Glaser_Diagram_Project", target='_blank')]),
    ],style={'textAlign': 'center', 'width': '70%', 'display': 'block', 'margin': '0 auto', 'fontFamily': 'Quicksand'}),
    html.Br(),
...
def update_graph(data, layer_boundaries_real, table_data):
    if data is not None:
        ...
        fig.update_yaxes(range=[y_min, y_max], title_text="Temperature [°C]", showgrid=False, secondary_y=False,
                         title_font={'family': 'Quicksand', 'size': 19},
                         tickfont={'family': 'Quicksand', 'size': 15})
